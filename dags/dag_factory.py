...
for data_object_id, data_object_definition in data_obj_spec.items():
    ...
    with DAG(
        ...
        def consumer_function(message, **kwargs):
            ...
            consumer_logger.info(f"{data_object_id}: {message.topic()} @ {message.offset()}; {key} : {value}")
        ...
        update_data = BashOperator(task_id="update-data", bash_command="echo $xcom", env={'xcom': '{{ ti.xcom_pull(task_ids="check-upstream-new-data") }}'})
            ...
            def update_dependencies(dependencies):
                ...
                tasks = [
                    TriggerDagRunOperator(
                        trigger_dag_id=d, task_id=f"update-{d}", wait_for_completion=True
                    )
